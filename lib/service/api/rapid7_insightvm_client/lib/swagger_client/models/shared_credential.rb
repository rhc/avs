  ...
  class SharedCredential
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::SharedCredential` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::SharedCredential`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'host_restriction')
        self.host_restriction = attributes[:'host_restriction']
      end
      ...
      if attributes.key?(:'port_restriction')
        self.port_restriction = attributes[:'port_restriction']
      end

      if attributes.key?(:'site_assignment')
        self.site_assignment = attributes[:'site_assignment']
      end
