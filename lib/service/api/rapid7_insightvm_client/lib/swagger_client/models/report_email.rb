...
module SwaggerClient
  class ReportEmail
    ...
    # Whether to distribute the report to all users to have access to assets in the report.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::ReportEmail` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::ReportEmail`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'additional_recipients')
        if (value = attributes[:'additional_recipients']).is_a?(Array)
          self.additional_recipients = value
        end
      end

      if attributes.key?(:'asset_access')
        self.asset_access = attributes[:'asset_access']
      end
    ...
    def valid?
      access_validator = EnumAttributeValidator.new('Object', ['file', 'zip', 'url', 'none'])
      return false unless access_validator.valid?(@access)
      additional_validator = EnumAttributeValidator.new('Object', ['file', 'zip', 'none'])
      return false unless additional_validator.valid?(@additional)
      owner_validator = EnumAttributeValidator.new('Object', ['file', 'url', 'zip', 'none'])
    ...
    def access=(access)
      validator = EnumAttributeValidator.new('Object', ['file', 'zip', 'url', 'none'])
    ...
    def additional=(additional)
      validator = EnumAttributeValidator.new('Object', ['file', 'zip', 'none'])
    ...
    def owner=(owner)
      validator = EnumAttributeValidator.new('Object', ['file', 'url', 'zip', 'none'])
