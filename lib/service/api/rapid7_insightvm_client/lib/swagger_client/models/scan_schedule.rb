  ...
  class ScanSchedule
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::ScanSchedule` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::ScanSchedule`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'next_runtimes')
        if (value = attributes[:'next_runtimes']).is_a?(Array)
          self.next_runtimes = value
        end
      end

      if attributes.key?(:'on_scan_repeat')
        self.on_scan_repeat = attributes[:'on_scan_repeat']
      end
      ...
      if attributes.key?(:'scan_engine_id')
        self.scan_engine_id = attributes[:'scan_engine_id']
      end
      ...
      if attributes.key?(:'scan_template_id')
        self.scan_template_id = attributes[:'scan_template_id']
      end
