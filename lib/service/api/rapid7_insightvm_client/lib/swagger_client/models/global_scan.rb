...
module SwaggerClient
  class GlobalScan
    ...
    # ${scan.username}
    attr_accessor :started_by_username
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::GlobalScan` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::GlobalScan`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'engine_ids')
        if (value = attributes[:'engine_ids']).is_a?(Array)
          self.engine_ids = value
        end
      end
      ...
      if attributes.key?(:'scan_name')
        self.scan_name = attributes[:'scan_name']
      end
      ...
      if attributes.key?(:'started_by_username')
        self.started_by_username = attributes[:'started_by_username']
      end
    ...
    def valid?
      status_validator = EnumAttributeValidator.new('Object', ['aborted', 'unknown', 'running', 'finished', 'stopped', 'error', 'paused', 'dispatched', 'integrating'])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('Object', ['aborted', 'unknown', 'running', 'finished', 'stopped', 'error', 'paused', 'dispatched', 'integrating'])
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          started_by_username == o.started_by_username &&
          status == o.status &&
    ...
    def hash
      [assets, duration, end_time, engine_id, engine_ids, engine_name, id, links, message, scan_name, scan_type, site_id, site_name, start_time, started_by, started_by_username, status, vulnerabilities].hash
    end
