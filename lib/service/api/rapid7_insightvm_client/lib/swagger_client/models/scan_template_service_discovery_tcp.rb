  ...
  class ScanTemplateServiceDiscoveryTcp
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::ScanTemplateServiceDiscoveryTcp` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::ScanTemplateServiceDiscoveryTcp`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'additional_ports')
        if (value = attributes[:'additional_ports']).is_a?(Array)
          self.additional_ports = value
        end
      end

      if attributes.key?(:'excluded_ports')
        if (value = attributes[:'excluded_ports']).is_a?(Array)
    ...
    def valid?
      method_validator = EnumAttributeValidator.new('Object', ['SYN', 'SYN+RST', 'SYN+FIN', 'SYN+ECE', 'Full'])
      return false unless method_validator.valid?(@method)
      ports_validator = EnumAttributeValidator.new('Object', ['all', 'well-known', 'custom', 'none'])
    ...
    def method=(method)
      validator = EnumAttributeValidator.new('Object', ['SYN', 'SYN+RST', 'SYN+FIN', 'SYN+ECE', 'Full'])
    ...
    def ports=(ports)
      validator = EnumAttributeValidator.new('Object', ['all', 'well-known', 'custom', 'none'])
